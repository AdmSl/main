...
extern "C" void MatterPostAttributeChangeCallback(const chip::app::ConcreteAttributePath & attributePath,
                                       uint8_t type, uint16_t size, uint8_t * value)
{
    // ZMIANA: Sprawdzamy teraz Endpoint 3 (nasza nowa lampa)
    if (attributePath.mEndpointId == 3 && attributePath.mClusterId == OnOff::Id) 
    {
        if (attributePath.mAttributeId == OnOff::Attributes::OnOff::Id) 
        {
            bool newValue = *value;
...
OnOffLightSwitchDevice *mySwitch = nullptr;
OnOffLightDevice *myLight = nullptr;


void RegisterMyModbusEndpoint() {
    // 1. Tworzymy obiekty urządzeń
    mySwitch = new OnOffLightSwitchDevice("modbus_switch_1", "Modbus Wall Switch");
    myLight = new OnOffLightDevice("modbus_light_1", "Modbus Gate Light");

    if (!mySwitch || !myLight) {
        printk("BŁĄD: Brak pamięci RAM na alokację urządzeń mostka!\n");
        return;
    }

    // 2. Tablica urządzeń przekazywana do managera z flagą Nrf::
    Nrf::MatterBridgedDevice *devicesArray[] = { mySwitch, myLight };
    uint8_t devicesCount = 2;
    uint8_t pairIndexes[2] = { 0, 0 };

    // 3. NAPRAWA TUTAJ: Tworzymy właściwego dostawcę (bez przedrostka Nrf::)
    auto *simulatedProvider = chip::Platform::New<SimulatedOnOffLightDataProvider>(
        Nrf::BridgeManager::HandleUpdate, 
        Nrf::BridgeManager::HandleCommand
    );
    // 4. Wywołujemy oficjalną rejestrację w BridgeManagerze
    CHIP_ERROR err = Nrf::BridgeManager::Instance().AddBridgedDevices(
        devicesArray, 
        simulatedProvider, 
        devicesCount, 
        pairIndexes
    ...
    if (err == CHIP_NO_ERROR) {
        printk("SUCCESS: Urządzenia Modbus pomyślnie zarejestrowane w BridgeManager!\n");
    } else {
        printk("BŁĄD: Rejestracja w BridgeManager nieudana (Kod: %" CHIP_ERROR_FORMAT ")\n", err.Format());
    }
}
...
extern "C" {
    ...
    void MatterGroupKeyManagementPluginServerInitCallback(void) {}
}
